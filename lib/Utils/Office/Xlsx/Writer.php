...
class Writer
{
    ...
    protected ?bool $isRightToLeft = null;
    protected ?string $company = null;
    protected ?string $description = null;
    protected array $keywords = [];
    protected ?string $tempdir = null;

    protected ?string $current_sheet = null;
    protected array $sheets = [];
    protected array $temp_files = [];
    protected array $cell_styles = [];
    protected array $number_formats = [];
    ...
    public function setKeywords(string|array $keywords = ''): void { $this->keywords = $keywords; }
    public function setDescription(string $description = ''): void { $this->description = $description; }
    public function setTempDir(string $tempdir = ''): void { $this->tempdir = $tempdir; }
    public function setRightToLeft(bool $isRightToLeft = false): void { $this->isRightToLeft = $isRightToLeft; }
    ...
    protected function initializeSheet($sheet_name, $col_widths=array(), $auto_filter=false, $freeze_rows=false, $freeze_columns=false )
    {
        ...
        $this->sheets[$sheet_name] = (object)array(
            ...
            'row_count' => 0,
            'file_writer' => new Buffer($sheet_filename),
            'columns' => array(),
            'merge_cells' => array(),
        ...
        $max_cell = writer::xlsCell(self::EXCEL_2007_MAX_ROW, self::EXCEL_2007_MAX_COL);//XFE1048577
    ...
    public function writeSheetHeader(string $sheet_name, array $header_types, ?array $col_options = null): void
    {
    ...
    public function writeSheetRow(string $sheet_name, array $row, ?array $row_options = null): void
    {
        if (empty($sheet_name))
            return;
        ...
        if (count($sheet->columns) < count($row)) {
            ...
            $sheet->columns = array_merge($sheet->columns, $default_column_types);
        }
    ...
    public function countSheetRows(string $sheet_name = ''): int
    {
    ...
    public function writeSheet(array $data, string $sheet_name = '', array $header_types = []): void
    {
    ...
    protected function writeCell(Buffer &$file, int $row_number, int $column_number, mixed $value, string $num_format_type, int $cell_style_idx): void
    {
        ...
        if (!is_scalar($value) || $value==='') { //objects, array, empty
            ...
            if (!is_string($value) || $value=='0' || ($value !== '' && $value[0]!='0' && ctype_digit($value)) || preg_match("/^\-?(0|[1-9][0-9]*)(\.[0-9]+)?$/", $value)){
                if(strlen($value) < 9)
                {
                    $file->write('<c r="'.$cell_name.'" s="'.$cell_style_idx.'" t="n"><v>'.self::xmlspecialchars($value).'</v></c>');
                }//int,float,currency
                else
                {
                    $file->write('<c r="'.$cell_name.'" s="'.$cell_style_idx.'" t="inlineStr"><is><t>'.self::xmlspecialchars($value).'</t></is></c>');
    ...
    protected function styleFontIndexes()
    {
        ...
        foreach($this->cell_styles as $i=>$cell_style_string)
        {
            ...
            if (isset($style['border']) && is_string($style['border']))//border is a comma delimited str
            {
                ...
                if (isset($style['border-color']) && is_string($style['border-color']) && $style['border-color'] !== '' && $style['border-color'][0]=='#')
                {
            ...
            if (isset($style['fill']) && is_string($style['fill']) && $style['fill'] !== '' && $style['fill'][0]=='#')
            {
            ...
            if (isset($style['color']) && is_string($style['color']) && $style['color'] !== '' && $style['color'][0]=='#' )
            {
    ...
    protected function writeStylesXML()
    {
        ...
        $file = new buffer($temporary_filename);
     ...
     * */
    public static function xlsCell(int $row_number, int $column_number, bool $absolute = false): string
    {
        $n = $column_number;
    ...
    //------------------------------------------------------------------
    public static function log(string|array $string): void
    {
    ...
    //------------------------------------------------------------------
    public static function sanitize_filename(string $filename): string //http://msdn.microsoft.com/en-us/library/aa365247%28VS.85%29.aspx
    {
    ...
    //------------------------------------------------------------------
    public static function sanitize_sheetname(string $sheetname): string
    {
        static $badchars  = '\\/?*:[]';
        static $goodchars = '        ';
    ...
    //------------------------------------------------------------------
    public static function xmlspecialchars(mixed $val): string
    {
    ...
    //------------------------------------------------------------------
    public static function array_first_key(array $arr): mixed
    {
        reset($arr);
        $first_key = key($arr);
        return $first_key;
    ...
    //------------------------------------------------------------------
    private static function determineNumberFormatType(string $num_format): string
    {
    ...
    //------------------------------------------------------------------
    private static function numberFormatStandardized(string $num_format): string
    {
    ...
    //------------------------------------------------------------------
    public static function add_to_list_get_index(array &$haystack, mixed $needle): int
    {
    ...
    //------------------------------------------------------------------
    public static function convert_date_time(mixed $date_input): float //thanks to Excel::Writer::XLSX::Worksheet.pm (perl)
    {
