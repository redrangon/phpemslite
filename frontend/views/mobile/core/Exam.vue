<template>
	<div style="width:100%;">
		<!-- 导航条 -->
        <van-nav-bar title="我的考试" fixed left-arrow  placeholder @click-left="$router.go(-1)"/>
			...
			<van-space direction="vertical" fill>
				<van-list v-if="page.total > 0" v-model="loading" :finished="finished" finished-text="没有更多了" @load="onLoad">
                    <van-cell-group v-if="exams.length >= 1" class="menu-list">
                        <van-cell v-for="(basic, index) in exams" :key="index" :is-link="true" :title="basic.basic" center title-style="flex: 1;min-width: 0;" @click="setExam(basic.basicid)">
                            <template #label>
...
export default {
	...
	methods:{
		...
		onLoad:async function() {
			// 加载更多数据
			let data = await userApi.getMyBasic({
		...
		setExam:async function(examId){
			await this.execute( async () =>{
				...
				if(this.store.basic.basicexam.model === 2)
                {
                    this.$router.push('/mobile/exam/exam');
                }
				else
                {
