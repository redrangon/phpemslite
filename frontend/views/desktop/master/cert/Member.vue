<template>
	...
	<lay-card>
		<lay-table id="cemid" ref="tableRef" v-model:selected-keys="selectedKeys" :columns="columns" :data-source="dataSource" :default-toolbar="false">
			<template #toolbar>
			...
			<template v-slot:cemstatus="{row}">
				<span v-if="row.cemstatus === 1">已审核</span>
				<span v-else>待审核</span>
			</template>
			<template #footer>
                <lay-button :disabled="selectedKeys.length < 1" size="sm" type="primary" @click="verifyMember()">通过审核</lay-button>
                <lay-button :disabled="selectedKeys.length < 1" size="sm" type="primary" @click="delMember()">移除成员</lay-button>
			...
			<template v-slot:operator="{ row }">
				...
				<lay-button size="xs" type="danger" @click="delMember(row.cemid)">移出</lay-button>
			</template>
		</lay-table>
	</lay-card>
			...
			<lay-table id="mpassport" ref="utableRef" v-model:selectedKeys="memberSelectedKeys" :columns="memberColumns" :data-source="usersSource">
				<template #footer>
					<lay-row>
						<lay-col md="12">
                            <lay-space>
                                <lay-button size="sm" type="primary" @click="addMember">下发证书</lay-button>
                            </lay-space>
						</lay-col>
						<lay-col md="12">
		...
		<div style="padding: 20px;">
			<table class="table">
                ...
                <tbody>
                    ...
                    <tr>
                        <th>手机号</th>
                        <td colspan="2">{{ member.mphone }}</td>
                        <th>地址</th>
                        <td colspan="3">{{ member.maddress }}</td>
                    </tr>
                    <tr>
			...
			<lay-tab v-model="searchType" type="brief">
				<lay-tab-item id="files" title="个人经历">
					<table class="table">
						...
						<tr>
							<td colspan="7">
								<img :src="member.mpassportimg" style="margin:5px;max-width: 96%">
							</td>
						</tr>
						<tr>
...
export default {
	...
	async mounted() {
		this.ceId = this.$route.params.ceid;
		await this.getData()
	},
	components:{
		myUploadFile,
	...
	methods:{
		getData:async function(){
			await this.execute(async () => {
				const data = await certApi.getMemberList({
                    ceId:this.ceId,
					search:this.search,
		...
		addMember:async function(){
            await this.base(async () => {
                ...
                await certApi.addMemberByPassport({
                    ceId:this.ceId,
                    mids:this.memberSelectedKeys
                });
            });
		},
		delMember:function(cemid){
            this.confirmDelete(async () => {
                await certApi.deleteMember(cemid?[cemid]:this.selectedKeys);
            },this.getData);
		},
		showPageData:async function(row){
			this.member = await memberApi.getMember({passport:row.cempassport});
			this.showPage = true
		},
        refreshNumber:function(){
            this.confirmOperate('确定要刷新吗？',async () => {
                await certApi.refreshNumber(this.ceId);
            });
        },
        verifyMember:function(){
            this.confirmOperate('确定要通过审核吗？',async () => {
                await certApi.verifyMember(this.selectedKeys);
            },this.getData);
        },
