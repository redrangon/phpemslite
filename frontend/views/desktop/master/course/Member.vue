					...
					<lay-row>
						<lay-col md="12">
                            <lay-space>
                                ...
                                <lay-button size="sm" type="primary" @click="addCourseMember">开通课程</lay-button>
                            </lay-space>
						</lay-col>
						<lay-col md="12">
	...
	methods:{
		getData:async function(){
			await this.execute(async () => {
				const data = await courseApi.getMemberList({
					csId:this.csId,
					search:this.search,
        ...
        refreshStats:async function(){
            await this.base(async () => {
                ...
                let data;
                while(status){
                    data = await courseApi.refreshMemberStats({
                        csId:this.csId,
		...
		addCourseMember:async function(){
            await this.base(async () => {
                this.showAddPage = false;
                await courseApi.addMemberByPassport({
                    csId:this.csId,
		...
		delCourseMember:function(cmid){
            this.confirmDelete(async () => {
                await courseApi.deleteMember([cmid]);
            },this.getData);
		},
