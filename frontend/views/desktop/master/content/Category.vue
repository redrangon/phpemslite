<template>
	<lay-card class="pagecontent">
		<lay-table :columns="columns" :data-source="dataSource" v-model:selectedKeys="selectedKeys" even>
			...
			<template #catimg="{ row }">
				<lay-avatar :src="row.catimg?row.catimg:'@/assets/images/upload.png'" size="lg"></lay-avatar>
			</template>
			<template #toolbar>
	...
	methods:{
		getData:async function(){
			await this.execute(async () => {
				const data = await contentApi.getCategoryList({
		...
		parentCate:function(catid){
			this.execute(async () => {
				this.parentid = catid;
				this.currentcat = await contentApi.getCategory(catid);
		...
		addCategory:function(){
			this.base(async () => {
				await contentApi.addCategory(this.model);
				this.model = {};
			}, '添加成功');
		},
		...
		modifyCategory:function(){
			this.base(async () => {
				await contentApi.modifyCategory(this.modify);
			}, '修改成功');
		},
		modifySequence:function(row){
			this.base(async () => {
				await contentApi.modifyCategory({
					catid:row.catid,
					catlite:row.catlite
				});
			}, '修改成功');
		...
		deleteData:function(id){
			this.confirmDelete(async () => {
				await contentApi.delCategory(id?[id]:this.selectedKeys);
			}, this.getData);
		}
	}
}
