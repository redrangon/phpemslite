	...
	<lay-card>
		<lay-space direction="vertical">
			<lay-space size="lg">
				<lay-space></lay-space>
				<lay-space>
					<span style='width:70px'> ID：</span><lay-input v-model="search.questionid"></lay-input>
				</lay-space>
				<lay-space>
				...
				<lay-space>
					...
					<lay-select v-model="search.questiontype" allow-clear placeholder="请选择" style="min-width: 180px;width:100%">
			...
			<lay-space size="lg">
				...
				<lay-space>
					...
					<lay-select v-model="search.level" allow-clear placeholder="请选择" style="min-width: 180px;width:100%">
						<lay-select-option v-for="(level,lid) in levels" :key="lid" :label="level" :value="lid"></lay-select-option>
					</lay-select>
				</lay-space>
				<lay-space>
...
export default {
	...
	data() {
		return {
			...
			search:{},
			selectedKeys:[],
			selectors:['A','B','C','D','E','F','G','H','I','J','K'],
			questionTypes:{},
			layout:['count', 'prev', 'page', 'next', 'limits',  'refresh', 'skip'],
			page:{limit:20,current:1,total:0},
			showAddPage:false,
	...
	async mounted() {
		if(this.$route.query.questionid)this.search.questionid = this.$route.query.questionid;
	...
	methods:{
		...
		showModify:function(row){
			const question = {...row};
			if(this.questionTypes[question.questiontype].questsort === 0 && (this.questionTypes[question.questiontype].questchoice === 2 || this.questionTypes[question.questiontype].questchoice === 3))
			{
		...
		batDelQuestions:function(){
			withConfirm('删除仅删除与知识点关联，试题不删除，确定要操作吗？',async () => {
		...
		addQuestion:function(){
			this.base(async() => {
				await examApi.addQuestion(this.model)
			});
		},
