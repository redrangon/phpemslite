<template>
	<lay-space direction="vertical" fill wrap class="login_box">
		<h2 style="text-align: center;color:#FFFFFF;font-size:18px;">找回密码</h2>
		<lay-form :model="model" :pane="false" size="md" labelWidth="80" class="form" ref="forgetForm">
			<template v-if="hasCode">
				...
				<lay-form-item>
					<lay-space direction="vertical" fill wrap size="md">
						<lay-button type="normal" fluid @click="submit">找回密码</lay-button>
						<p style="text-align: center;">
							<router-link style="color:#ffffff;" to="/desktop/home/auth/login">已有密码，立即登录</router-link>
						</p>
					</lay-space>
				</lay-form-item>
			</template>
...
const submit = async function () {
	forgetForm.value.validate().then(async (res) => {
		try{
			if(hasCode.value)
			{
			...
			{
				await authApi.getFindPasswordCode({
					username:model.value.username,
					useremail:model.value.useremail,
